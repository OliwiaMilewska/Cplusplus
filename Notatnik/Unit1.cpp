...
{
   if(Application->MessageBox("Czy na pewno zakoñczy program?","PotwierdŸ",MB_YESNO | MB_ICONQUESTION)==IDYES)
  {
...
{
   if(Application->MessageBox("Czy na pewno wyczyscic plik?","PotwierdŸ",MB_YESNO | MB_ICONQUESTION)==IDYES)
  {
   tresc->Lines->Clear();
   }
}
...
{
  if(Application->MessageBox("Czy na pewno zakoñczy program?","PotwierdŸ",MB_YESNO | MB_ICONQUESTION)==IDNO)
  {
   Action=caNone;
  }
}
